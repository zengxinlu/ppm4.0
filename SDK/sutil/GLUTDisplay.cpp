...
unsigned int   GLUTDisplay::m_timed_frames         = 1100u;//30u;
...
void GLUTDisplay::display()
{
	if (m_cur_continuous_mode == CDNone) {
		sutil::displayBufferPPM("output.ppm", m_scene->getOutputBuffer()->get());
		quit();
	}
...
void GLUTDisplay::quit(int return_code)
{
	try {
		if(m_scene)
		{
			...
			{
				sutil::reportErrorMessage( "Derived scene class failed to call SampleScene::cleanUp()" );
				exit(2);
			}
		}
		exit(return_code);
