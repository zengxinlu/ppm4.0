...
double *Area_CUDA;
Point *st_CUDA, *d2_CUDA;
int *stpos, *stpos_CUDA;
...
__global__ void calcAreaOne(PointWithID *d, int *g, EdgeData *edge, Frag *frag, double *Area, Point *st, Point *d2, int *stpos)
{
...
extern "C" void calcArea_GPU(int n, int m, PointWithID *d, int *g, EdgeData *edge, Frag *frag, double *Area, int total, int *edgeCount, int maxcnt)
{
	checkCudaErrors(cudaMalloc((void **) &Area_CUDA, (n + 1) * sizeof(double)));
	
	checkCudaErrors(cudaMalloc((void **) &st_CUDA, (total + 7 * n) * sizeof(Point)));
	checkCudaErrors(cudaMalloc((void **) &d2_CUDA, (total + 7 * n) * sizeof(Point)));

	stpos = (int*) malloc((n + 1) * sizeof(int));
	if (stpos == NULL) { fprintf(stderr, "No Memory"); exit(0); }
	checkCudaErrors(cudaMalloc((void **) &stpos_CUDA, (n + 1) * sizeof(int)));
	...
	for (int i = 1; i < n; ++i)
		stpos[i + 1] = stpos[i] + edgeCount[i] + 7;

	checkCudaErrors(cudaMemcpy(stpos_CUDA, stpos, (n + 1) * sizeof(int), cudaMemcpyHostToDevice));
	...
	calcAreaOne<<<dimGrid, maxThreads>>>(d_CUDA, g_CUDA, edge_CUDA, frag_CUDA, Area_CUDA, st_CUDA, d2_CUDA, stpos_CUDA);
	...
	checkCudaErrors(cudaFree(st_CUDA));
	checkCudaErrors(cudaFree(d2_CUDA));
	checkCudaErrors(cudaFree(stpos_CUDA));
}
