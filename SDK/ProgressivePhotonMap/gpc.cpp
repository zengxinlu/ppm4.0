...
void get_area(gpc_tristrip *polygon, float *m_area)
{
	*m_area = 0;
	if (!polygon->num_strips)
		return;
	if (polygon->num_strips != 1)
	{
		int my_debug = 0;
	}
	for (int i = 0;i < polygon->strip->num_vertices - 2;i ++)
	{
		gpc_vertex A = polygon->strip->vertex[i];
		gpc_vertex B = polygon->strip->vertex[i + 1];
		gpc_vertex C = polygon->strip->vertex[i + 2];
		*m_area += abs(A.x*(B.y-C.y) + B.x*(C.y-A.y) + C.x*(A.y-B.y))/2.0;
	}
}
